import sys
from PyQt5.QtWidgets import QDialog, QApplication
from demoRadioButton1 import *

class MyForm(QDialog):
    def __init__(self):
        super().__init__()
        self.initUI()
    def initUI(self):
        self.ui = Ui_Dialog()
        self.ui.setupUi(self)
        self.ui.radioButtonFirstClass.toggled.connect(self.dispFare)
        self.ui.radioButtonBusinessClass.toggled.connect(self.dispFare)
        self.ui.radioButtonEconomClass.toggled.connect(self.dispFare)
        self.show()

    def dispFare(self):
        fare = 0
        if self.ui.radioButtonFirstClass.isChecked() == True:
            fare = 150
            print(fare)
        if self.ui.radioButtonBusinessClass.isChecked() == True:
            fare = 125
            print(fare)
        if self.ui.radioButtonEconomClass.isChecked() == True:
            fare = 100
            print(fare)
        self.ui.labelFare.setText("Тариф на самолёт" + str(fare))


if __name__=='__main__':
    app = QApplication(sys.argv)
    w = MyForm()
    sys.exit(app.exec_())